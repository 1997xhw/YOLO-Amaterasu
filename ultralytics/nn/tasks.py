...
###  xuhaowen
from ultralytics.nn.head.SC_C_Detect import SC_C_Detect
from ultralytics.nn.head.Partial_C_Detect import Detect_improve
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        for m in self.model:
            if isinstance(m, (Detect,Detect_improve,Detect_improve, SC_C_Detect, Segment, Pose)):
                s = 256  # 2x min stride
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect,Detect_improve,Detect_improve, SC_C_Detect, Segment):
...
def attempt_load_one_weight(weight, device=None, inplace=True, fuse=False):
    ...
    for m in model.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Detect_improve,SC_C_Detect, Segment):
            m.inplace = inplace  # torch 1.7.0 compatibility
        elif t is nn.Upsample and not hasattr(m, 'recompute_scale_factor'):
            m.recompute_scale_factor = None  # torch 1.11.0 compatibility
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        #### xuhaowen concatV2
        elif m is Concat_dropoutV2:
            # 计算拼接后的通道数
            c2 = ch[-1]
            # 从索引 f 中提取每个要拼接的特征图的通道数
            ch_list = [ch[x] for x in f]
        #### xuhaowen concatV2
        ...
        elif m in (Detect, SC_C_Detect, Segment, Pose, RTDETRDecoder):
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Detect, SC_C_Detect):
                return 'detect'
